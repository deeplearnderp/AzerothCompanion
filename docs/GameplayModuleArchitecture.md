...
**Interrupts:** Direct combat-log tracking is intentionally disabled on Retail 12.0.7. Historical interrupt fields remain schema-compatible and currently record zero; supported normalized interrupt facts may return only through the shared `CombatSessionService`, without Mythic+-specific combat listeners.
...
#### Purpose
Reports Great Vault reward-slot progress. Deliberately narrow — "Weekly" as a blanket concept (all weekly-reset content) is not a single Blizzard system and would not have one natural owner; this module exists specifically for the one clean, aggregate API Blizzard already provides. It exposes Dungeon, Delve/World, and Raid categories when Blizzard returns them while preserving the established Dungeon projection for existing Mythic+ consumers.

#### Responsibilities
- Great Vault reward-slot progress by Blizzard category: ordered slots, threshold/progress/remaining, qualifying level, unlock state, reward preview or generated-item metadata, next upgrade information, and whether an unclaimed reward is waiting.
...
#### Blizzard APIs owned
`C_WeeklyRewards`: `GetActivities`, `HasAvailableRewards`, `GetItemHyperlink`, `GetExampleRewardItemHyperlinks`, `GetNextMythicPlusIncrease`, `GetNextActivitiesIncrease`. `C_Item`: `GetItemInfo`, `GetDetailedItemLevelInfo`. `WEEKLY_REWARDS_UPDATE`, `WEEKLY_REWARDS_ITEM_CHANGED`, `GET_ITEM_INFO_RECEIVED`.

**VERIFICATION STATUS (Blizzard API Verification Workflow pass):** `WeeklyRewardActivityInfo`'s field shape (`type`, `index`, `threshold`, `progress`, `id`, `activityTierID`, `level`, `claimID`, `raidString`, `rewards`) and the `WEEKLY_REWARDS_UPDATE` event name are both confirmed directly against Blizzard's own FrameXML source (`Blizzard_WeeklyRewards/Blizzard_WeeklyRewards.lua`, mirrored at `Gethe/wow-ui-source`). This pass also found and fixed a real bug: `activity.level` is the Mythic+ KEY level (e.g. "15"), not an item level — the module previously exposed this to Home's "Highest Reward" card mislabeled. Reward item metadata now comes from `GetActivityReward()`, which reproduces Blizzard's own resolution chain (`activity.rewards` → `C_Item.GetItemInfo` → `C_WeeklyRewards.GetItemHyperlink` → `C_Item.GetDetailedItemLevelInfo`) and returns `nil` rather than guessing when the item cache is not ready; the module listens for `GET_ITEM_INFO_RECEIVED` and refreshes later. Preview rewards and upgrade thresholds come from `GetExampleRewardItemHyperlinks`, `GetNextMythicPlusIncrease`, and `GetNextActivitiesIncrease`. **Still Needs Live Verification:** whether `threshold`/`progress` are counted in whole dungeons completed on the current build — conceptually confirmed via documentation and a cross-referenced community addon (`Broker_GreatVault`), but not literally observed against a live client's numbers. See `docs/DEVELOPMENT_BACKLOG.md`'s Critical section for the full citation list and the Live Verification Checklist.

#### Dashboard responsibilities
Home page "Vault Progress" card (`X / Y Slots`, plus a note when a reward is unclaimed) — clickable as of the Technical Debt & Completion sprint, navigating to a dedicated Weekly page. The Weekly page enumerates module-provided categories and renders every returned slot through the shared Vault overview component. Guidance remains on Home and the Recommendations destination so the Weekly page stays a focused, glanceable Vault-status surface.
...
#### Out of Scope
PvP-specific presentation and reward claiming remain out of scope. Per-activity history *toward* a vault slot remains owned by its gameplay module; Weekly only reports Blizzard's current aggregate slot/threshold/reward view.
...
#### Purpose
Makes verification fast without turning "add logging" into the answer to everything. A permanent developer console with eleven pages grouped under Console, Runtime, Blizzard APIs, and Data & Maintenance (`Core/UI/DeveloperPanel.lua`) plus the backing `Core/Services/DeveloperModeService.lua` and `Core/Services/VerificationService.lua` — rather than one-off debug prints scattered through gameplay modules.
...
A standalone `BaseWindow` (same tier as `DiagnosticsWindow`/`SettingsWindow`), presentation-only throughout. Its two-level grouped navigation keeps page count independent from window width without narrowing the full-width inspector content. Every value traces to an existing public getter, `DeveloperModeService`'s own observation state, `AC.DeveloperRuntime`'s capabilities, or `VerificationService`'s registry/log:
...
- **Modules** — one row per registered Service/Module with dynamically discovered capabilities. Registration is the only lifecycle fact the managers own, so the page does not infer initialization or enablement. Refresh and Diagnostics are listed only as supported capabilities; provider-specific diagnostic payloads remain on their owning pages. Restart, reinitialize, and enable/disable are intentionally absent because lifecycle managers do not model per-object dependency order, state transitions, or idempotency.
- **Combat Session** — the production inspection surface for `CombatSessionService`, reading only `GetDiagnostics()` and `GetLatestSession()`. It presents lifecycle/API status, cache/update/reset statistics, and normalized participants/deaths; Refresh is the only page action and exports reuse the global copy footer.
...
- **Secret Values** — presents the `SecretValueEvents` runtime capability through the same shared accordion and page-toolbar infrastructure as Errors.
...
- **API Explorer** — generic read-only runtime Blizzard API inspection, deliberately separate from Live API's curated verification probes.
...
- **Maintenance** — owns only the global reset-all-developer-data action. Page-specific destructive actions remain with Events, Errors, Secret Values, Checklist, and History; empty Analytics/Telemetry/Cache/Export placeholders were removed.
...
- **Toolbar** — the shared page-toolbar infrastructure presents persisted Capture intent, `ErrorCapture:IsInstalled()` ground truth, error count, and the page-local Clear/Generate/Export actions. Its status region anchors to the rendered action chain rather than reserving a page-specific fixed width.
...
**Historical stabilization note — capture was silently failing.** Live testing found `/ac dev testerror` correctly triggering Blizzard's own error popup while the Errors tab stayed at 0. The capture pipeline was hardened with protected signature computation, failure logging, and a message-only fallback. The Generate Test Error action and separate Capture/Installed status remain. The toolbar layout described by the original stabilization pass has since been retired: the current console uses grouped navigation, Overview-owned global actions, and shared page-local toolbar infrastructure.
...
Every page supports Copy JSON / Copy Text / Copy Summary (`DeveloperModeService`'s generic serializers, DiagnosticsWindow's established "focus + highlight an EditBox, player presses Ctrl+C" idiom — WoW has no clipboard-write API). `CopyCurrentTab` normalizes both plain and colored summary-line shapes before concatenating. The former persistent action row was removed: Force Refresh and tracing already belong to Overview/slash-command workflows, while notification clearing now sits with Overview's notification diagnostics. Common page-local action rows use one shared toolbar helper.
...
Section 1.4's own Future Extension Points entry on "Party composition" explicitly deferred storing facts about other players, reasoning that full named specs would need the heavier `NotifyInspect`/`INSPECT_READY` path, and that "this needs a deliberate decision before any code is written here, not a default." That deferral was scoped to `MythicPlusModule` specifically. `PlayerJournalModule` owns party-member identity and relationship facts using basic roster reads (`UnitFullName`/`UnitGUID`/`UnitClass`/`UnitGroupRolesAssigned`). Unsupported live combat-log tracking is intentionally disabled; deaths and interrupts will return only through the future shared `CombatSessionService`, which will remain the single owner of supported Retail combat-session facts.
...
**Run-tracking event flow:** `CHALLENGE_MODE_START` snapshots the party roster (self excluded); `GROUP_ROSTER_UPDATE` runs a grace-period leave check (a roster member missing for 5 seconds, with the run still active, is marked left-early — an addon-side heuristic, not a documented Blizzard behavior, flagged `pj.rosterLeaveDetection`); `CHALLENGE_MODE_COMPLETED` snapshots this module's own state, then defers the actual finalization one frame via `C_Timer.After(0, ...)`. Live deaths and interrupts are intentionally not collected here while the shared `CombatSessionService` is incomplete; Player Journal must consume that shared owner rather than register combat events itself.
...
#### UI design
**Ordering — oldest-to-newest**, a deliberate, explicit departure from every other Dashboard list (all newest-first): this page tells a story arc, not "what's new."
